...
void ModelOptions::load(const char* modeloptions_file)
{
	...
	fin>>nmax>>fixed;
	fin>>fluxlo_min>>fluxlo_max>>fluxhi_min;
	fin>>fluxnorm>>norm_min>>norm_max;
	fin>>bg_min>>bg_max;
	...
	cout<<"# Using flux lower limit between "<<fluxlo_min<<" and "<<fluxlo_max<<" ph cm^-2 s^-1"<<endl;
...
MyRJObject<MyDistribution> ModelOptions::objects()
{
	...
	return MyRJObject<MyDistribution>(2+Data::get_instance().get_nbin(), nmax, fixed, MyDistribution(
			...
			data.get_bmin(), data.get_bmax(),
                        fluxlo_min, fluxlo_max, fluxhi_min, fluxnorm, norm_min, norm_max));
}
